...
export function EditCustomerModal({ customer, user, onClose, onSuccess }: EditCustomerModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    staff_in_charge_id: customer.staff_in_charge_id?.toString() || '',
    stage: customer.stage,
    level: customer.level,
    contact_status: customer.contact_status,
    customer_feedback: customer.customer_feedback || '',
    notes: customer.notes || '',
    appointment_date: customer.appointment_date ? new Date(customer.appointment_date).toISOString().slice(0, 16) : '',
    appointment_reminder: customer.appointment_reminder || ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await backend.crm.updateCustomer(submitData);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              {user.role === 'sales' ? (
                <Input
                  value={customer.staff_in_charge_name || user.name}
                  disabled
